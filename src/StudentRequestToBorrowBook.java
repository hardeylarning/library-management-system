 ...
 * @author HARDEXICO
 */
public class StudentRequestToBorrowBook extends javax.swing.JFrame {
Connection conn;
ResultSet rs,catRs;
...
Login mat = new Login();
 String matric_no=mat.userName;
    public StudentRequestToBorrowBook() {
        super("BORROW BOOK");
        initComponents();
...
//        searchForBook();
    ...
    //Search by Name
     ...
     public void borrowBook(){
         try{
                String sql="insert into stdRequestToBorrowBook(Book_id,Book_name,Edition,Rack_no,Category,Matric_no,Surname,Firstname,Phone_no,Address,Request_date) values(?,?,?,?,?,?,?,?,?,?,?)";
                pst=conn.prepareStatement(sql);
                ...
                pst.setString(6, matric_no);
                pst.setString(7, surname);
                pst.setString(8, firstname);
                pst.setString(9, phone_no);
           ...
           pst.close();
            if(i>=4){
                   JOptionPane.showMessageDialog(null, "You have Exceeded the book "
                           + "you can borrow in the Library because you have borrowed"
