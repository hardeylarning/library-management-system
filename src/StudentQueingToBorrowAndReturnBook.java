 ...
 * @author HARDEXICO
 */
public class StudentQueingToBorrowAndReturnBook extends javax.swing.JFrame {
 Connection conn;
    PreparedStatement pst, stPst;
    ResultSet rs;

//
    String book_id2, book_name2, edition2, author2, isbn2, pages2, rack2, cat2,publisher2, issued_date;
    int to_return_date;
    String matric, surname, firstname, dept, faculty, level, phone, address, request_date,get_to_return_date,rack_no;
    Sms sms = new Sms();
    /**
     * Creates new form StudentQueingToBorrowAndReturnBook
     */
    public StudentQueingToBorrowAndReturnBook() {
        initComponents();
    ...
    //
     public void stdRequestToBorrowBook() {
        try {
            String sql = "select * from stdRequestToBorrowBook ORDER BY stb_id DESC ";
    ...
    public void insertToBorrowReceivedMessage() {
        try {
            String insert = "insert into stdToBorrowReceivedMessage(Book_id,Book_name,Edition,";
                   insert+="Rack_no,Category,Matric_no,Surname,Firstname,Phone_no,Address,Request_date,Message_time)";
                   insert+="values(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public void insertToReturnReceivedMessage() {
        try {
            ...
            String insert = "insert into stdToReturnReceivedMessage(Matric_no,Surname,Firstname,Phone_no,";
   ...
   //  To return receive message
    ...
    public void issueBook() {
        ...
        try {
            ...
            pst.executeUpdate();
            pst.close();
            JOptionPane.showMessageDialog(null, "Book has been Successfully "
                    + "Issued Out To\n" + "Fullname: " + surname + " " + 
                    firstname + "\n with Matric No: " + matric + "\n Time: " 
                    + time);

            String update="UPDATE Book SET Availability=? where Book_id=? ";
            stPst=conn.prepareStatement(update);
            stPst.setString(1, "no");
            stPst.setString(2, book_id2);
            stPst.executeUpdate();
            stPst.close();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
    ...
    //Sent me
    //
    public void stdRequestToReturnBook() {
        try {
            String sql = "select * from stdRequestToReturnBook ORDER BY rtrb_id DESC ";
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
    ...
    public void returnBook() {
        try {
            String insert = "insert into stdReturnedBook(Matric_no,Surname,"
                    + "Firstname,Phone_no,Address,Book_id,";
            ...
            String update="UPDATE Book SET Rack_no=? , Availability=? where "
                    + "Book_id=? ";
            stPst=conn.prepareStatement(update);
            stPst.setString(1, rack_no);
            stPst.setString(2, "yes");
            stPst.setString(3, book_id2);
            stPst.executeUpdate();
            stPst.close(); 
    ...
    public void Delete(String table_name) {
        String sql = "delete from '"+table_name+"' where Matric_no=? AND Book_id=?";
    ...
    public void updateToYes(){
        ...
        try {
            stPst=conn.prepareStatement(sql);
            stPst.setString(1, "yes");
            stPst.setString(2, book_id2);
            stPst.executeUpdate();
            stPst.close();
        }
    ...
    private void initComponents() {
        ...
        borrow_table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Db Id", "Book_ID", "Book Name", "Edition", "Rack No", "Category", "Matric No", "Surname", "Firstname", "Phone No", "Address", "Request Date"
            }
        ));
        ...
        jPanel1.add(jScrollPane7, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 40, 1250, 490));
        ...
        jPanel1.add(jButton8, new org.netbeans.lib.awtextra.AbsoluteConstraints(1180, 540, -1, -1));

        jTabbedPane1.addTab("Students Queing to Borrow Details", jPanel1);
        ...
        jTable2.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Db_id", "Matric No", "Surname", "Firstname", "Phone No", "Address", "Book Id", "Book Name", "Edition", "Category", "Issue Date", "To Return Date", "Request Date"
            }
        ) {
        ...
        jTabbedPane1.addTab("Student Queing to Return Details", jPanel2);
        ...
        setSize(new java.awt.Dimension(1306, 697));
    ...
    private void borrow_tableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_borrow_tableMouseClicked
        ...
        matric = model.getValueAt(i, 6).toString();
        ...
        String acceptance_message = "Dear " + surname + " " + firstname + " You "
        + "can now come to Library for " + book_name2 + " you requested to borrow";
        String rejected_message = "Dear " + surname + " " + firstname + " Your "
                + "Requested to borrow " + book_name2 + " was rejected. You can "
                + "contact Library for more details";

        int response = JOptionPane.showConfirmDialog(this, "Do you want to "
                + "continue ? \nYES to send message either to come for pick up "
                + "or to\n Reject their request \n "
            + "NO to Issue Book without message \n CANCEL to go back without any"
                + " action ", "Confirm", JOptionPane.YES_NO_CANCEL_OPTION, 
                JOptionPane.QUESTION_MESSAGE);
        if (response == JOptionPane.YES_OPTION) {
            int action=JOptionPane.showConfirmDialog(this, "Do you want to "
            ...
            if (action == JOptionPane.YES_OPTION) {
                ...
                Delete("stdRequestToBorrowBook");
                stdRequestToBorrowBook();

            } else if (action == JOptionPane.NO_OPTION) {
                int option=JOptionPane.showConfirmDialog(this, "Do you want to "
                        + "continue? \n YES to reject request with rejection "
            ...
            int action = JOptionPane.showConfirmDialog(this, "Do you want to "
                    + "Issue Book out without sending message to "
                + "\n" + surname + " " + firstname+" ? \n YES to Issue Book \n "
                        + "NO to cancel", "Confirm",JOptionPane.YES_NO_OPTION, 
                        JOptionPane.QUESTION_MESSAGE);
            if (action == JOptionPane.YES_OPTION) {
                String return_date=JOptionPane.showInputDialog("Enter Expected "
                        + "date to return book");
    ...
    private void jTable2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable2MouseClicked
        ...
        try {
            ...
            matric = model.getValueAt(i, 1).toString();
            ...
            to_return_date =Integer.parseInt(model.getValueAt(i, 11).toString());
            request_date = model.getValueAt(i, 12).toString();

            String text_message = "Dear " + surname + " " + firstname + " "
                    + "You can now come to Library for " + book_name2 + " "
            ...
            if (response == JOptionPane.YES_OPTION) {
                ...
                int action = JOptionPane.showConfirmDialog(this, "Do you want to"
                        + " collect Returned Book without sending message to \n"
                        + surname + " " + firstname+" ?\n"
                    + "YES to collect Returned Book \nNo to CANCEL", "Confirm",
                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
                if (action == JOptionPane.YES_OPTION) {
                    rack_no=JOptionPane.showInputDialog("Enter Rack no to place "
                            + "the returned book");
                    returnBook();
                    Delete("stdRequestToReturnBook");
                    stdRequestToReturnBook();
