 ...
 * @author HARDEXICO
 */
public class StaffRequestToReturnBook extends javax.swing.JFrame {
Connection conn;
ResultSet rs,catRs;
PreparedStatement pst,catPst;
    //
String surname,firstname,phone_no,address,rack;
...
Login d = new Login();
 String user=d.userName;
    public StaffRequestToReturnBook() {
    ...
    //Search by Name
     public void showBorrowedBook() {
        try {
            String sql="select Book_id,Book_name,Edition,Category,Issued_date,To_return_date from stfIssuedBook where Staff_id =?";
        pst=conn.prepareStatement(sql); 
            pst.setString(1, user);
            rs=pst.executeQuery();
...
//        String bookId="",name_of_book="",book_edition="",category="",issued_date="",to_return_date="";
        ...
        if(response==JOptionPane.YES_OPTION){
            //        pst=conn.prepareStatement(sql); 
            String sql="insert into stfRequestToReturnBook(Staff_id,Surname,Firstname,Phone_no,Address,Book_id,Book_name,Edition,"
                    + "Category,Issued_date,To_return_date,Request_date) values(?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
             info();
            pst=conn.prepareStatement(sql);
            pst.setString(1, user);
            ...
            pst.executeUpdate();
            pst.close();
            JOptionPane.showMessageDialog(null, "Book is being submitted for return\n You will receive message from Library for fully return\n Thank you");
            String delete="delete from stfIssuedBook where Staff_id=? AND Book_id=? ";
